...
class DictRemoveGUI(tkSimpleDialog.Dialog) :
	def __init__(self, parent, DBobj, title=None) :
		self.DBobj = DBobj
		tkSimpleDialog.Dialog.__init__(self, parent, title)

	def body(self, parent) :
		tki.Label(parent,text="Select a dictionary to remove:").grid(row=0,column=0,sticky=tki.W,padx=5)
	...
	def UpdateDictDropDown(self,NewList) :
		"""
		Updates the values available in the dictionary drop down menu.
		The user should select the dictionary to remove from the database

		NewList - a list of tuples to define entrys for the dictionary drop down menu.  Each tuple is:
...
if __name__ == "__main__" :
	...
	root = tki.Tk()
	root.wm_title("Test the dictionary removal dialog")
	EndConnection = DictRemoveGUI(root,aa,"Remove a dictionary")
	if EndConnection.result is not None :
		logging.debug("Dictionary id to remove: %s"%str(EndConnection.result))
	else :
		logging.debug("No dictionary id returned")
